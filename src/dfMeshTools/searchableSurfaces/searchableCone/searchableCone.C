...
) const
{
    ...
    v = v/mag(v); //this is based on the Vector primitive in v2006
...
:
    ...
    unitDir_((point2_-point1_)/magDir_)//wrap up the lookup function with read scalar
{
    bounds() = calcBounds();
}
...
:
    ...
    innerRadius1_(dict.lookupOrDefault<scalar>( "innerRadius1", 0.0)),
    point2_(dict.lookup("point2")),
    radius2_(readScalar(dict.lookup("radius2"))),
    innerRadius2_(dict.lookupOrDefault<scalar>( "innerRadius2", 0.0)),
    magDir_(mag(point2_-point1_)),
    unitDir_((point2_-point1_)/magDir_) //wrap up the lookup function with read scalar
